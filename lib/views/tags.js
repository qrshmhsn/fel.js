...
//if_end
templater.registerHelper('if_and', function(left, right, options) {
  if (left && right)
    return options.fn(this);
  return options.inverse(this);
});
//if_or
templater.registerHelper('if_or', function(left, right, options) {
  if (left || right)
    return options.fn(this);
  return options.inverse(this);
});
...
//loop
templater.registerHelper('iter', function(context, options) {
  ...
  if(context && context.length > 0) {
    for(var i=0, j=context.length; i<j; i++) {
        ...
        if(!_.isObject(ctx))
          ctx = {context: ctx};
        ret = ret + fn(_.extend({}, ctx, { index: i, first: first, last: last }));
    }
  } else {
    ret = inverse(this);
  }
