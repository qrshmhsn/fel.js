...
var Viewer = SingletonObject.extend({
	...
	__init__: function(app, options){
		...
		self.bindings = {};
		//bind tag
		//usage: {@bind var="app.var1"/}
		//or: {@bind var="app.var1" attr="class"/}
		dust.helpers["bind"] = function(chunk, context, bodies, params){
			var variable = params["var"];
			var attr = params.attr;
			var result = self.bind(variable, attr);
			return chunk.write(result);
		};
		//usage: {@render var="app.var1" template="main"/}
	...
	bind: function(variable, attr, template){
		...
		var watch_obj = root_obj[first_part];
		if(!last_part){
			watch_obj = root_obj;
