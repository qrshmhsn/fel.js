...
var Viewer = SingletonObject.extend({
	...
	__init__: function(app, options){
		...
		for(var name in self.options.templates.tags){
			var tag = self.options.templates.tags[name];
			templater.registerHelper(name, tag); 
		}
		for(var name in self.options.templates.filters){
			var filter = self.options.templates.filters[name];
			self.filters[name] = filter;
		}
		self.refresh();
		self.translator = new views.Translator(self.options.translations);
		viewer = self;
		if(self.eventer)
...
var View = Object.extend({
	...
	toString: function(){
		return viewer.templates[this.name](this.context);
	}
});
